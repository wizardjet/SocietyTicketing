...
class DBHandler:
    ...
    def create_tables(self):
        ...
        table_event_attendee = f"CREATE TABLE IF NOT EXISTS smig_event_attendee (event_id INT, person_email VARCHAR({self.EMAIL_MAX_CHAR}), amount_paid DECIMAL(10,2), CONSTRAINT FOREIGN KEY (person_email) REFERENCES smig_person (email), CONSTRAINT FOREIGN KEY (event_id) REFERENCES smig_event (id) ON DELETE CASCADE)"
    ...
    def del_person(self, person):
        ...
        if self.exists_person(person):
            del_row = "DELETE FROM smig_person WHERE email={person.email}"
            self.query(del_row)
